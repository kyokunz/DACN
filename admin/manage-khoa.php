...
if (strlen($_SESSION['sturecmsaid'] == 0)) {
  ...
  if (isset($_GET['delid'])) {
    ...
    $sql = "delete from tbl_khoa where id=:rid";
    $query = $dbh->prepare($sql);
    $query->bindParam(':rid', $rid, PDO::PARAM_INT);
    $query->execute();
    echo "<script>alert('Data deleted');</script>";
    echo "<script>window.location.href = 'manage-khoa.php'</script>";
  }
?>
                    ...
                    <div class="table-responsive border rounded p-1">
                      <table class="table">
                        ...
                        <tbody>
                          ...
                          $ret = "SELECT id FROM tbl_khoa";
                          $query1 = $dbh->prepare($ret);
                          $query1->execute();
                          $results1 = $query1->fetchAll(PDO::FETCH_OBJ);
                          $total_rows = $query1->rowCount();
                          $total_pages = ceil($total_rows / $no_of_records_per_page);
                          $sql = "SELECT * from tbl_khoa LIMIT $offset, $no_of_records_per_page";
                          $query = $dbh->prepare($sql);
                          $query->execute();
                          ...
                          if ($query->rowCount() > 0) {
                            foreach ($results as $row) {               ?>
                              <tr>
                                ...
                                <td>
                                  <div>
                                    <a class="btn btn-info btn-sm mr-2" href="edit-khoa-detail.php?editid=<?php echo htmlentities($row->id); ?>">
                                      <i class="icon-eye"></i>
                                    </a>
                                    <a class="btn btn-sm btn-danger" href="manage-khoa.php?delid=<?php echo ($row->id); ?>" onclick="return confirm('Bạn có chắc muốn xóa ?');"> <i class="icon-trash"></i></a>
                                  </div>
                                </td>
                              </tr><?php $cnt = $cnt + 1;
                                  }
